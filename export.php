...
if ($shid == 0)
  ...
  $shedulerarray = array("schedulerid"=>$shid);
  ...
  foreach($shedulers as $sheduler){
    ...
    if (!empty($appointmentarray[$sheduler->id]->id)) {
      ...
      if (empty($users[$appointment->studentid])) {
        ...
        if ($groups = $DB->get_records_sql("SELECT g.id, g.name, u.username, u.firstname, u.lastname FROM {groups} g, {groups_members} gm, {user} u WHERE g.courseid =? AND gm.groupid = g.id AND g.name NOT LIKE 'Make%' AND gm.userid = ?", array($cid, $appointmentarray[$sheduler->id]->studentid))) {
          $groupstitle  = "";
          foreach($groups as $group){
            $groupstitle .= $group->name.",";
            
            if ($group->id == $gid)
              $activeg = true;
          }
          $groupstitle = substr($groupstitle, 0, -1);
          $users[$appointmentarray[$sheduler->id]->studentid]->group = $groupstitle;
        } else
          $users[$appointmentarray[$sheduler->id]->studentid]->group = "--";
      }
      
      if ($gid != 0 && $activeg == false)
        $active = false;
      ...
      //echo $active ." / ".$shedulerdataarray[$sheduler->schedulerid]->course ."==". $cid."<br />";
